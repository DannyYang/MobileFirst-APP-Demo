...
//                       ];
    id cancelButton = [[UIBarButtonItem alloc]
                       initWithTitle:@"返回" style:UIBarButtonItemStyleBordered target:(id)self action:@selector(cancelButtonPressed:)
                       ];
    ...
    toolbar.items = [NSArray arrayWithObjects:cancelButton,flexSpace ,shutterButton,nil];
#else
    toolbar.items = [NSArray arrayWithObjects:cancelButton,flexSpace,nil];
#endif
    ...
    CGRect  rectArea       = CGRectMake(0, 0, rootViewWidth, toolbarHeight+21);
